...
p('A extração com Python e Beautiful Soup foi corrigida e validada em um HTML renderizado presente no projeto. Foram recuperados <b>10 resultados únicos</b> (9 cards de notícia e 1 de vídeo), com título, URL, resumo e data exibida. Os <b>23 testes automatizados</b> passaram na verificação desta entrega.')
...
('Base validada','10 registros; 1 lote; 0 duplicados observados'),
('Campos de publicação','0/10 com data de publicação comprovada; campo preservado como null'),
...
p('O JS referencia pagination.nextPage, scrollCount e IntersectionObserver; o botão chama o carregamento de mais resultados. Isso aponta para lotes incrementais, mas não prova um endpoint, cursor nem que o parâmetro page da URL seja aceito.')
p('Para concluir a aquisição: observar uma interação real; capturar HTML e horário antes/depois; comparar URLs; fornecer o DOM renderizado ao mesmo parser. Repetir até fim explicitamente observado, limite configurado, falha ou ausência de novos IDs. Timeout e ausência de novos IDs devem ser estados de interrupção, não prova automática de esgotamento.')
...
p('Escopo: 10 registros da referência, 10 registros extraídos, 1 lote. Métricas reproduzidas por evaluate.py; resultados em data/metricas.json. Denominador zero produz null, nunca 100%.')
...
('Integridade: hashes conferidos / registros','10/10 = 100%','Arquivos locais correspondem ao texto analisado.'),
...
p('<b>Validade:</b> amostra pequena, por conveniência, sem revisão humana independente e sem outros lotes. Os percentuais de 100% não podem ser extrapolados. Uma notícia antiga não é, por si só, um dado coletado desatualizado.',True)

page(8,'Testes e proposta de uso de LLM')
h('Verificação automatizada executada')
p('<b>23 passed in 0.29s</b>, Python 3.14.4, pytest 9.1.1. Evidência: evidence/tests_verificacao.txt. Os testes incluem parsing e semântica de datas; campos ausentes; quarentena; HTML vazio; múltiplos lotes; deduplicação; URLs de tracking; Unicode; timeout, conexão, HTTP e Content-Type; métricas; vídeo e regressão do snapshot real.')
p('Os dois testes adicionados nesta revisão validam detecção de adulteração de snapshot e preservação dos dados após falha de um lote. Não foi executado teste de integração da paginação ao vivo; testes sintéticos não comprovam seu funcionamento.')
...
# Reproduzir a extração real do snapshot incluído
python scraper.py --html evidence/Busca.html --out data/repro
...
p('As barras invertidas no comando de avaliação indicam continuação em shells Unix. No PowerShell, execute esse comando em uma única linha. Linux/macOS: ativar com source .venv/bin/activate.',True)
table(['Arquivo / pasta','Conteúdo'],[
('scraper.py / evaluate.py','Rotina modular de extração e avaliador reproduzível.'),
('tests/test_scraper.py','23 casos executados, incluindo parametrizações.'),
('data/coleta/','10 registros em JSON e CSV; HTML, log, eventos e quarentena.'),
('data/referencia_revisada.json / data/metricas.json','Referência provisória e métricas calculadas.'),
